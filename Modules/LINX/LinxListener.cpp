...
LinxListener::LinxListener()
{
    ...
    this->AttachCustomCommand(CUSTOM_CMD_OSTANOV - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_OSTANOV_f);
    this->AttachCustomCommand(CUSTOM_CMD_STATUS - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_STATUS_f);
    this->AttachCustomCommand(CUSTOM_CMD_RUN - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_RUN_f);
    this->AttachCustomCommand(CUSTOM_CMD_PID - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_PID_f);
    this->AttachCustomCommand(CUSTOM_CMD_CALIBRATION - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_CALIBRATION_f);
}
	...
	****************************************************************************************/	
	if(command >= 0xFC00)	
	{
		...
		if (command >= CMD_OFFSET) {
			CALL_MEMBER_FN(this->customCmdHandler, customCommands[command - CMD_OFFSET])(commandPacketBuffer[1]-7, commandPacketBuffer+6, &numResponseBytes, responsePacketBuffer+5);
		}
	...
	else
	{
		...
		************************************************************************************/
		...
		case 0x0004: //Get LINX API Version
			...
			PacketizeAndSend(commandPacketBuffer, responsePacketBuffer, 4, L_OK); //3 ???
			break;
		...
		case 0x0006: //Set UART Listener Interface Max Baud
		{
			...
			//unsigned long actualBaud = 115200;
			...
			#ifdef ASK_BLuePill_Pinout
			unsigned long targetBaud = (unsigned long)((unsigned long)(commandPacketBuffer[6] << 24) | (unsigned long)(commandPacketBuffer[7] << 16) | (unsigned long)(commandPacketBuffer[8] << 8) | (unsigned long)commandPacketBuffer[9]);
			unsigned long actualBaud = huart2.Init.BaudRate;
			#else
			unsigned long targetBaud = 0;
			unsigned long actualBaud = 0;
			#endif

			responsePacketBuffer[5] = (actualBaud>>24) & 0xFF;
			responsePacketBuffer[6] = (actualBaud>>16) & 0xFF;
			responsePacketBuffer[7] = (actualBaud>>8) & 0xFF;
			responsePacketBuffer[8] = actualBaud & 0xFF;
			PacketizeAndSend(commandPacketBuffer, responsePacketBuffer, 4, L_OK);
			break;
		}		
		...
		case 0x000A: // Get AO Channels
			...
			responsePacketBuffer[5] = 0;
			PacketizeAndSend(commandPacketBuffer, responsePacketBuffer, 2, L_OK); //1
		   break;
		...
		case 0x000D: // Get UART Channels
		   DataBufferResponse(commandPacketBuffer, responsePacketBuffer, LinxDev->UartChans, LinxDev->NumUartChans, L_OK);
			//responsePacketBuffer[5] = 1;
			//PacketizeAndSend(commandPacketBuffer, responsePacketBuffer, 1, L_FUNCTION_NOT_SUPPORTED);
		   break;
		...
		****************************************************************************************/	
		...
		case 0x0082:	//PWM Set Frequency
		{
			status = LinxDev->PwmSetFrequency(commandPacketBuffer[6], &commandPacketBuffer[7], (unsigned long*)(&commandPacketBuffer[8]));
			StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
			break;
		}
...
//}
void LinxListener::AttachCustomCommand(unsigned short commandNumber, LinxStatus (LinxCustomCmdHandler::*function)(unsigned char, unsigned char*, unsigned char*, unsigned char*) ) {
    customCommands[commandNumber] = function;
}
